<?php
	...
	class InvoiceAll extends FPDF {
		...
		public function buildReport($data){
			...
			$w = array(10, 25, 20, 15, 15, 30, 30, 15, 30);
			$this->Ln(5);
			...
			foreach($data['rows'] as $row){
				...
				for($i=0;$i<$rowCount;$i++) {
					...
					$this->Cell($w[7],$tHeight,isset($dataValue[$i][6]) ? $dataValue[$i][6] : ' ',$merge,0,'R');
					$this->Cell($w[8],$tHeight,isset($dataValue[$i][6]) ? $dataValue[$i][6] : ' ',$merge,0,'R');
