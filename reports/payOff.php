<?php
	...
	class PayOff extends FPDF {
		...
		public function buildReport($data){
			$w = array(10, 20, 20, 20, 30, 40, 20, 10, 15, 40, 25, 25);
			...
			$dateStr = Utils::dateIDFormat(isset($data['startDate']) ? $data['startDate'] : date("Y/m/d"), 1);
			...
			$this->Cell($w[0]+$w[1]+$w[2]+$w[3]+$w[4]+$w[5]+$w[6],7,'Total','LRB',0,'C');
			$this->Cell($w[7],7,number_format($data['sumTotalColli']),'LRB',0,'R');
			$this->Cell($w[8],7,number_format($data['sumTotalWeight']),'LRB',0,'R');
			$this->Cell($w[9],7,'Rp. '.number_format($data['sumPrice'],2,',','.'),'LRB',0,'R');
		}
